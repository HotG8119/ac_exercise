...
const friends = JSON.parse(localStorage.getItem("bestFriends"));
let filterFriends = [];
...
function deleteBestFriend(id) {
  const friendIndex = friends.findIndex(friend => friend.id === id);
  friends.splice(friendIndex, 1);

  localStorage.setItem("bestFriends", JSON.stringify(friends));
...
cardContainer.addEventListener("click", event => {
  ...
  if (event.target.id === "heart") {
    deleteBestFriend(Number(event.target.dataset.id));
